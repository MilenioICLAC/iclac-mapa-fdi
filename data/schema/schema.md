...
| `cancelled` | enum | opt | `0` vigente \| `1` cancelada | **Atributo de la inversión: mismo valor en todas sus filas.** `1` saca la inversión del dataset principal y del mapa, y la manda al anexo. El ETL corta **por inversión**: si una entrega trae el valor a medias, la inversión entera va al anexo, porque partirla mandaría medio trazado a cada archivo. Estuvo fuera del contrato hasta v1.5, y mientras tanto el informe la reportaba como «columna extra que el sistema ignora». |
| `cancelled_motivo` | texto | opt | | Por qué quedó fuera. Texto libre, y **viaja a las dos descargas**, porque es lo único que distingue una cancelación real de una exclusión por otra razón. En la entrega del 15-08 conviven ahí «cancelado», «evidencia insuficiente (score <3)», «duplicado de X», «fuera de alcance» y «en estudio»: son cinco decisiones distintas bajo una sola marca. |
