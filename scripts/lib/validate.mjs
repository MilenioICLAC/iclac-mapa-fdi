...
// Opcionales cuya AUSENCIA igual se avisa, una vez por archivo. No entran a
// REQUIRED_SOFT_COLUMNS porque esa lista además valida celda por celda, y estas
// son atributo de la inversión, no del punto: una inversión de 5.000 filas dejaría
// 5.000 avisos del mismo hueco. El aviso es de archivo y nunca bloquea.
...
export const validateRows = (rows, opts = {}) => {
  ...
  for (const [col, mensaje] of Object.entries(SUGGESTED_COLUMNS)) {
    if (!columns.includes(col)) push('warning', 'archivo/columna-sugerida-ausente', 0, col, null, mensaje)
  }
